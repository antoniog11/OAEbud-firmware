...
bool nav_milestone_reached(float distance){
  ...
  //  return distance <= (POS_THRESHOLD/2.0);
  //}else if(m_current_milestone_idx == 0){
    return distance <= POS_THRESHOLD;
  //}else{
  //  return nav_crossed_milestone_plane(m_current_milestone_idx);
  //}
}
...
void nav_acc_update(void){
  ...
  // Robot moves forward along -y direction
  point acc;
  ...
  //NRF_LOG_RAW_INFO("[nav] Current accel (mg): ("NRF_LOG_FLOAT_MARKER", " NRF_LOG_FLOAT_MARKER") \n",
  //    NRF_LOG_FLOAT(acc.x),
  //    NRF_LOG_FLOAT(acc.y))
 ...
 //   NRF_LOG_FLOAT(pos_update.y))
  ...
  NRF_LOG_RAW_INFO("[nav] Current position (cm): ("NRF_LOG_FLOAT_MARKER", " NRF_LOG_FLOAT_MARKER") \n",
      NRF_LOG_FLOAT(current_state.position.x),
      NRF_LOG_FLOAT(current_state.position.y));
  
  NRF_LOG_RAW_INFO("[nav] Current velocity (cm/s): ("NRF_LOG_FLOAT_MARKER", " NRF_LOG_FLOAT_MARKER") \n",
      NRF_LOG_FLOAT(current_state.velocity.x),
      NRF_LOG_FLOAT(current_state.velocity.y))
