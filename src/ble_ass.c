...
static void on_write(ble_cus_t * p_cus, ble_evt_t const * p_ble_evt){  
  ...
  if (p_evt_write->handle == p_cus->characteristic_handles[ASS_CONTROL_CHAR_HANDLE].value_handle) {
    ...
    //}
      ...
      // probe calibration and TEOAE measurement

      if(data[1] == 0){
        
        NRF_LOG_INFO("Calibrating TEOAE");
        buzzer_set_chirp_scale(1);
        eventQueuePush(EVENT_CALIBRATION);

      }
      else if (data[1] == 1){
          int i;
          for(i=2;i<6; i++){
            NRF_LOG_INFO("Data: %d!", data[i]);
          }
        
          float pulse_scale = *((float*) (p_evt_write->data+2));
          //NRF_LOG_INFO("pulse_scale:"NRF_LOG_FLOAT_MARKER, NRF_LOG_FLOAT(pulse_scale));
          buzzer_set_chirp_scale(&pulse_scale);
          //NRF_LOG_INFO("pulse_scale:"NRF_LOG_FLOAT_MARKER, NRF_LOG_FLOAT(pulse_scale));
          eventQueuePush(EVENT_TEOAE_MEASURE);
      }
      ...
      params.mode = data[8];
      params.use_compression = data[9];
      params.ratio = data[10];
