...
bool ble_cus_send_indication(ble_cus_t *p_cus, const ble_gatts_char_handles_t * const handles, uint8_t * data, uint16_t length){
  if(!indication_in_progress && ble_cus_send_hvx(p_cus, handles, data, length, true)){
    //NRF_LOG_INFO("[ble_cus] Sent indication");
    indication_in_progress = true;
    return true;
  }
  //NRF_LOG_INFO("Indication status: %d", indication_in_progress);
  return false;
}
