...
/*
For future reference, in case time sync isn't working all of a sudden:
1- Make sure TIME_SYNC_DISABLED is set to 0
2- Make sure frequencies are exact everywhere 
(Specifically in chirp computation, and packet master initial packet dropping)
3- Make sure there's nothing wrong on the host side (streams, unwanted flushing ...)
*/
...
// "Resamples" buffer of length l1 to whatever sampling rate will make it have
// length l2
void audio_linear_interpolate(int16_t *buffer, int16_t l1, int16_t l2){
  //if(l1 == l2) return;
  //memcpy_fast(interp_table, buffer, sizeof(int16_t) * l1);
  //float idx, val;
  //int f;
  //buffer[l2-1] = interp_table[l1-1];
  //for(int i = l2 - 2; i > 0; i--){
  //  idx = (i * (l1 - 1.0) / (l2 - 1.0) );
  //  f = (int) idx;
  //  val = (idx - f) * (interp_table[(f+1 == l1 ? f : f + 1)] - interp_table[f]) + interp_table[f];
  //  buffer[i] = (int16_t) val + 0.5; // round
  //}
  //NRF_LOG_RAW_INFO("%d %d\n", l1, l2);
  //for(int i = 0; i < l2; i++) NRF_LOG_RAW_INFO("%d ", buffer[i]);
  //NRF_LOG_RAW_INFO("\n");
}
...
static void pdmEventHandler(nrfx_pdm_evt_t *event)
{
  ...
  if (event->buffer_released) {
    ...
    //NRF_LOG_RAW_INFO("[audio] Length before resampling: %d\n", length);
    //audio_resample(releasedPdmBuffer, buffer_length, (int16_t) round(PDM_BUFFER_DURATION * 16));
    //length = (int16_t) round(PDM_BUFFER_DURATION * 16);
    //NRF_LOG_RAW_INFO("[audio] Length after resampling: %d\n", length);
    ...
    if(m_use_compression){
      ...
      //bufferTweakAmount += (current_opus_window_size - length); // Resample to expected opus window size
      
      //audio_linear_interpolate(releasedPdmBuffer, length + bufferTweakAmount, current_opus_window_size);
      rb_put(&opus_rb, (uint8_t*) releasedPdmBuffer, sizeof(int16_t) * length * channels);
      ...
      while(rb_size(&opus_rb) >= 2 * current_opus_window_size * channels){
        ...
        int16_t nbytes = opus_encode(encoder, releasedPdmBuffer, current_opus_window_size, encoded_buffer, PACKET_LENGTH);
        if(nbytes < 0){
          ...
          ready = audio_queue_fill(&length, 1);
          ready |= audio_queue_fill(encoded_buffer, length);

          //int i = 0;
          //for(; i < nbytes; i++){
          //  audio_queue_put_sample(encoded_buffer[i]);
          //}
          //audio_queue_idx = PACKET_LENGTH;
          //ready |= audio_queue_put_sample(nbytes);
          //ASSERT(ready);
        }
      }
    }else{
      //if(bufferTweakAmount != 0){
      //  char msg[1];
      //  if(bufferTweakAmount > 0) msg[0] = '+';
      //  else msg[0] = '-';
      //  ble_indicate_status(DCS_STATUS_LOG, msg, 1);
      //}
      //audio_linear_interpolate(releasedPdmBuffer, length, length+bufferTweakAmount);
      ready = audio_queue_fill((uint8_t *) releasedPdmBuffer, sizeof(int16_t) * length * channels);
      //for(int i = 0; i < length; i++){
      //  // TODO: Clean (Optimize with repeated memcpy_fast calls?)
      //  uint8_t *sample = (uint8_t *) &releasedPdmBuffer[i];
      //  audio_queue_put_sample(sample[0]);
      //  audio_queue_put_sample(sample[1]);
        
      //  //NRF_LOG_RAW_INFO("[audio] idx %d PL %d\n", audio_queue_idx, PACKET_LENGTH);
      //  if(audio_queue_idx == PACKET_LENGTH){
      //    ready |= audio_queue_put_sample(PACKET_LENGTH);
      //  }
      //}
...
void audio_ready(void){
  ...
  if(m_use_compression){
    ...
    err = opus_encoder_ctl(encoder, OPUS_SET_BITRATE(OPUS_BITRATE));
...
void audio_params_update(audio_params_t *params){
  ...
  if(params){
    ...
    #if AUDIO_PARAMS_DEBUG
      NRF_LOG_RAW_INFO("%08d [audio] Updating clock freq to: %d\n", systemTimeGetMs(), params->div_clk);
    #endif
