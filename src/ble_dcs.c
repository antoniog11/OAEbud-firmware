  ...
  if(p_evt_write->handle == p_cus->characteristic_handles[DCS_COMMAND_CHAR_HANDLE].value_handle){
      ...
      switch(data[0]){
        case DCS_COMMAND_SYNC_TASK_CREATE:{
          ...
          ts_sync_task_add(task_id, arm_interval + MASTER_OVERFLOW_TRIGGER_COUNT);
        }
        break;
        case DCS_COMMAND_SYNC_TASK_CREATE_RECURRING:{
          ...
          ts_sync_task_add_recurring(task_id, period, start_interval + MASTER_OVERFLOW_TRIGGER_COUNT, end_interval + MASTER_OVERFLOW_TRIGGER_COUNT);
        }
        break;
