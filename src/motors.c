...
void motors_adjust(float speed, float beta){
  pulsating = false;
  if(!motors_started) motors_start();
  //int16_t m1_speed = ((int16_t) m1.target_speed[m1.p] - (int16_t) m1.target_speed[m1.n]);
  //int16_t m2_speed = ((int16_t) m2.target_speed[m1.p] - (int16_t) m2.target_speed[m1.n]);

  //float arg = atan2(m1_speed, m2_speed);
  //arg += ANGLE_CORR;

  //if(!direction){
  //  m1_speed = m1_speed * cos(ANGLE_CORR) - m2_speed * sin(ANGLE_CORR);
  //  m2_speed = m1_speed * sin(ANGLE_CORR) + m2_speed * cos(ANGLE_CORR);
  //}else{
  //  m1_speed = m1_speed * cos(-ANGLE_CORR) - m2_speed * sin(-ANGLE_CORR);
  //  m2_speed = m1_speed * sin(-ANGLE_CORR) + m2_speed * cos(-ANGLE_CORR);
  //}
