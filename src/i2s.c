  ...
  if (nrf_i2s_event_check(NRF_I2S, NRF_I2S_EVENT_TXPTRUPD)){
    ...
    if(m_stream.current_repetition == m_stream.num_repetitions){
      ...
      *((volatile uint32_t *)0x4002503C) = 1;
      ...
      NRF_LOG_RAW_INFO(" [main] cr: %d\t mr: %d\n",m_stream.current_repetition, m_stream.num_repetitions );
      i2s_load_next_chunk();
    }
  }
}

bool i2s_load_stream(const int16_t * const buffer, uint16_t length, uint32_t repititions){
...
bool i2s_init(void){
 ...
 cfg.mck_setup = NRF_I2S_MCK_32MDIV16;
 cfg.ratio = NRF_I2S_RATIO_64X;
  // Sampling frequency =~ 48kHz
  //cfg.mck_setup = NRF_I2S_MCK_32MDIV21;
  //cfg.ratio = NRF_I2S_RATIO_32X;
  
  // Sampling frequency = 15625
// cfg.mck_setup = NRF_I2S_MCK_32MDIV8;
